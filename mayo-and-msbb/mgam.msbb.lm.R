...
file=args[1]
msbbgt=read.csv(paste0(file,".gt.csv"))
...
for (i in colnames(msbbgt)[1:1106]) {
m=lm(as.numeric(msbbgt[[i]])~sex+RIN+pmi+apoe4_allele+age_death+g1,data=msbbgt)
n=lm(as.numeric(msbbgt[[i]])~sex+RIN+pmi+apoe4_allele+age_death+g2,data=msbbgt)
o=lm(as.numeric(msbbgt[[i]])~sex+RIN+pmi+apoe4_allele+age_death+g3,data=msbbgt)
print(i)
